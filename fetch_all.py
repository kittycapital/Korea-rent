...
CSV_JEONSE_DIR = os.path.join(DATA_DIR, 'csv_jeonse')
MIN_AREA = 59
...
def load_csv_jeonse_file(filepath):
...
def load_all_csv_jeonse():
    """data/csv_jeonse/ 의 모든 CSV → (전세 통합, 월세 통합) 반환"""
    csv_files = sorted(glob.glob(os.path.join(CSV_JEONSE_DIR, '*.csv')))
    if not csv_files:
        print(f"  ⚠️ {CSV_JEONSE_DIR}/ 디렉토리에 CSV 파일이 없습니다 (전월세 건너뜀)")
        return [], []
    ...
    for filepath in csv_files:
        fname = os.path.basename(filepath)
        j_items, w_items = load_csv_jeonse_file(filepath)
...
def build_district_data(recent, alldata, months_6_set, outfile='district_top10.json', label='매매'):
    """구별 TOP 10 JSON 생성 (매매/전세/월세 공용)"""
    ...
    for key, t10 in top10_map.items():
        ...
        result["data"][key] = {
            "top10": [{
                ...
                **( {"deposit": it['deposit'], "monthly": it['monthly']}
                    if 'deposit' in it else {} )
            } for it in t10],
            "series": series,
            "avg": avg_pp,
            "deals": deal_count
...
def main():
    ...
    print("  (전국 구별 TOP 10 + 서울 TOP 20 + 전세/월세)")
    print("=" * 60 + "\n")

    os.makedirs(DATA_DIR, exist_ok=True)
    os.makedirs(CSV_DIR, exist_ok=True)
    os.makedirs(CSV_JEONSE_DIR, exist_ok=True)

    # ── Step 1: 매매 CSV 데이터 로드 ──
    print("Step 1: 매매 CSV 데이터 로드\n")
    ...
    # ── Step 2: 매매 대시보드 생성 ──
    print("Step 2: 매매 대시보드 생성\n")

    # 대시보드 1: 전국 구별 TOP 10 (매매)
    build_district_data(recent, alldata, months_6_set,
                        outfile='district_top10.json', label='매매')

    # 대시보드 2: 서울 TOP 20
    build_seoul_html(recent_seoul, alldata_seoul)

    # ── Step 3: 전월세 CSV 데이터 로드 ──
    print("\nStep 3: 전월세 CSV 데이터 로드\n")
    all_jeonse, all_wolse = load_all_csv_jeonse()

    if all_jeonse or all_wolse:
        recent_jeonse = [
        ...
        # ── Step 4: 전세/월세 대시보드 생성 ──
        print("Step 4: 전세/월세 대시보드 생성\n")
